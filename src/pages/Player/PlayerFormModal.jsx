...
//         if (formData[key]) {
//           formDataToSubmit.append('user[picture]', formData[key]);
//         }
//       } else {
//         formDataToSubmit.append(`user[${key}]`, formData[key]);
//       }
//     });
...
const PlayerFormModal = ({ isOpen, onClose, formData, handleInputChange, handleSubmit, currentPlayer, resetForm }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    handleSubmit(formDataToSubmit); // Pass the formData to the parent for submission
  };
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative z-30">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <InputField
            label="Picture"
            name="picture"
            value={formData.picture || ''} // Ensure value is always controlled
            onChange={handleInputChange}
            type='file'
            accept="image/*"
            placeholder="Upload a picture"
